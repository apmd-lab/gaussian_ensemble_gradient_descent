...
low_fidelity_setting = [2,3] # low-fidelity simulation setting (e.g. RCWA: number of harmonics, FDTD: mesh density, etc.)
high_fidelity_setting = [5,3] # high-fidelity simulation setting (e.g. RCWA: number of harmonics, FDTD: mesh density, etc.)
...
t_high_fidelity = 4.4 # high-fidelity simulation time in seconds
...
suffix = 'integrated_bandpass_filter_Ndim' + str(Nx) + 'x' + str(Ny) + '_D' + str(symmetry) + '_upsample' + str(upsample_ratio) + '_sigma' + str(sigma_ensemble_max) + '_coeffExp' + str(coeff_exp) + '_brush' + str(brush_size) + '_try' + str(n_seed+1)
...
if optimization_algorithm == 'conventional':
    ...
    T1 = time.time()
    optimizer.run(n_seed, suffix, maxiter, eta_ADAM=eta_ADAM, binary=args.binary, grayscale_beta_init=4.0, n_beta=4, load_data=args.load_data)
    T2 = time.time()
    if comm.rank == 0:
