...
#--------------------------------------------------------------------------
import FDTD_functions.waveguide_U_bend_MEEP as objfun

lam_min = 1.3
lam_max = 1.6
N_freq = 31

design_dim = np.array([1.2*lam_max,1.2*lam_max])
...
padding[waveguide_bottom + brush_size + int(waveguide_width_npix):waveguide_top + brush_size + int(waveguide_width_npix),:brush_size] = 1
padding[waveguide_bottom + brush_size - int(waveguide_width_npix):waveguide_top + brush_size - int(waveguide_width_npix),:brush_size] = 1
...
low_fidelity_setting = 1/2 # low-fidelity simulation setting (e.g. RCWA: number of harmonics, FDTD: mesh density, etc.)
high_fidelity_setting = 1/2 #2 # high-fidelity simulation setting (e.g. RCWA: number of harmonics, FDTD: mesh density, etc.)
...
t_high_fidelity = 17.7 # high-fidelity simulation time in seconds
...
t_fwd_bwd = 91.9
Ntrial = int(np.round(10/(t_fwd_bwd/t_high_fidelity))) # only for conventional
...
suffix = 'waveguide_U_bend_Ndim' + str(Nx) + 'x' + str(Ny) + '_D' + str(symmetry) + '_upsample' + str(upsample_ratio) + '_sigma' + str(sigma_ensemble_max) + '_coeffExp' + str(coeff_exp) + '_brush' + str(brush_size) + '_try' + str(n_seed+1)

if optimization_algorithm == 'conventional':
    optimizer = CONV.optimizer(Nx=Nx,
                               Ny=Ny,
                               Ntrial=Ntrial,
                               symmetry=symmetry,
                               periodic=periodic,
    ...
    T1 = time.time()
    optimizer.run(n_seed, suffix, maxiter, eta_ADAM=eta_ADAM, binary=args.binary, grayscale_beta_init=1.7**2, n_beta=5, load_data=args.load_data)
    T2 = time.time()
    if comm.rank == 0:
