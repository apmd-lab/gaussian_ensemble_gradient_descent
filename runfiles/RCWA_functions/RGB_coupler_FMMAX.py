...
class custom_objective:
    ...
    def set_accuracy(self, n_harmonic):
        ...
        idx3 = jnp.where((self.coeffs[:,0] == self.diff_order[2,0]) & (self.coeffs[:,1] == self.diff_order[2,1]))[0][0]
        self.idx_tgt = jnp.array([idx1, idx2, idx3])
        ...
        if self.incident_pol == 'TM':
            ...
            forward_amplitude_0_start = forward_amplitude_0_start.at[:,self.basis_expansion.num_terms,0].set(1)  # tm
    ...
    def _create_jitted_cost_fn(self):
        ...
        def _pure_cost_fn(density):
            ...
            if incident_pol == 'TM':
                ...
                forward_amplitude_0_start = forward_amplitude_0_start.at[:,expansion.num_terms,0].set(1)  # tm
            ...
            if incident_pol == 'TM':
                T_tgt = transmitted_power[jnp.arange(n_freq), idx_tgt, 0]
            elif incident_pol == 'TE':
                T_tgt = transmitted_power[jnp.arange(n_freq), idx_tgt + expansion.num_terms, 0]
    
            cost = -(jnp.mean(T_tgt**IPR_exponent) - 0.0) / (1 - 0.0)
    ...
    def get_diffraction_and_fields(self, x):
        ...
        if self.incident_pol == 'TM':
            T_tgt = transmitted_power[jnp.arange(self.n_freq), self.idx_tgt, 0]
        elif self.incident_pol == 'TE':
            T_tgt = transmitted_power[jnp.arange(self.n_freq), self.idx_tgt + self.basis_expansion.num_terms, 0]
        ...
        if self.incident_pol == 'TM':
            E_in_plane = jax.device_get(Ey)
        elif self.incident_pol == 'TE':
            E_in_plane = jax.device_get(Ex)
        x = jax.device_get(x)
        y = jax.device_get(y)
        z = jax.device_get(z)
