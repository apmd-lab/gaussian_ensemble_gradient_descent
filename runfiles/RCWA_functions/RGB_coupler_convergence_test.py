...
for nb in range(n_struct):
    ...
    for nhx in range(1, max_harmonic_x + 1):
        for nhy in range(1, max_harmonic_y + 1):
            print('\t\tHarmonics: x = ' + str(nhx) + ' | y = ' + str(nhy), end='', flush=True)
            cost_obj.set_accuracy([nhx,nhy])
            x = x_brush_all[nb,:].reshape(Nx*upsample_ratio, Ny*upsample_ratio)
    
            t1 = time.time()
            cost = cost_obj.get_cost(x, False)
            t2 = time.time()
            sim_time[nb,nhx-1,nhy-1] = t2 - t1
        
            cost_all[nb,nhx-1,nhy-1] = cost
            print(' | Fwd Time: ' + str(sim_time[nb,nhx-1,nhy-1]) + ' s', end='', flush=True)
        
            t1 = time.time()
            cost = cost_obj.get_cost(x, True)
            t2 = time.time()
            sim_time_AD[nb,nhx-1,nhy-1] = t2 - t1
