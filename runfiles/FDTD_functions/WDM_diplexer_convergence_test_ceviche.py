...
#--------------------------------------------------------------------------
import WDM_diplexer_ceviche as objfun

lam_tgt = np.array([1.53, 1.55])

design_dim = np.array([1.5, 1.5])
waveguide_width = 0.5
...
for nb in range(10):
    ...
    for nu in range(upsampling_ratio.size):
        ...
        np.savez("WDM_diplexer_convergence_test_ceviche_Nx" + str(Nx) + "_Ny" + str(Ny) + "_mfs" + str(min_feature_size),
            cost_all=cost_all,
