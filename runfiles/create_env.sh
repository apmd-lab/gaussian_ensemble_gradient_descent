...
conda run -n gegd_dev python3 -m pip install torch torchvision --index-url https://download.pytorch.org/whl/cu126
conda run -n gegd_dev python3 -m pip install -U "jax[cuda12]"
conda run -n gegd_dev python3 -m pip install --upgrade fmmax ceviche jeig