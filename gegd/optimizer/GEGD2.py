...
class optimizer:
    ...
    def ensemble_jacobian(self, x_bounded, test_function):
        ...
        for n in range(self.r_CV*self.Nensemble):
            f_ctrl_all[n] = self.cost_obj.get_cost(x_sample[n,:], False)
        f_mean = np.mean(f)
        f_std = np.std(f)
        f_norm = (f - f_mean) / f_std
        f_norm[f_norm<=0] = -np.exp(-self.coeff_exp * f_norm[f_norm<=0])
        f_norm[f_norm>0] = np.exp(self.coeff_exp * f_norm[f_norm>0])
        ...
        if np.all(f == np.mean(f)):

        self.coeff_exp = f_shifted
        f[f<=self.cost_threshold] = -np.exp(-self.coeff_exp * (f[f<=self.cost_threshold] - self.cost_threshold))
        f[f>self.cost_threshold] = np.exp(self.coeff_exp * (f[f>self.cost_threshold] - self.cost_threshold))
