...
def binarize(
    ...
    if method == 'brush':
        ...
        if min_feature_size is not None:
            if Nthreads > 0:
                if upsample_ratio == 1:
                    x_bin = FDG.make_feasible_parallel(
                        x_reward,
                        min_feature_size,
                        periodic,
                        symmetry,
                    ...
                    x_bin_lowres = FDG.make_feasible_parallel(
                        x_reward,
                        min_feature_size,
                        periodic,
                        symmetry,
                    ...
                    x_bin = FDG.make_feasible_parallel(
                        2*x_bin_lowres.astype(np.float32)-1,
                        min_feature_size,
                        periodic,
                        symmetry,
                        2,
                ...
                if data_size[comm.rank] > 0:
                    if upsample_ratio == 1:
                        x_bin_proc = FDG.make_feasible_parallel(
                            x_reward_proc,
                            min_feature_size,
                            periodic,
                            symmetry,
                        ...
                        x_bin_lowres_proc = FDG.make_feasible_parallel(
                            x_reward_proc,
                            min_feature_size,
                            periodic,
                            symmetry,
                        ...
                        x_bin_proc = FDG.make_feasible_parallel(
                            2*x_bin_lowres_proc.astype(np.float32)-1,
                            min_feature_size,
                            periodic,
                            symmetry,
                            2,
                    ...
                    x_bin_proc = np.array([]).astype(np.float64)
                
                if padding is None:
                    data_size_temp = data_size*Nx*Ny*upsample_ratio**2
                    data_disp_temp = np.array([sum(data_size_temp[:p]) for p in range(comm.size)])
                    
                    x_bin_temp = np.zeros(N_designs*Nx*Ny*upsample_ratio**2)
                    comm.Allgatherv(x_bin_proc.reshape(-1), [x_bin_temp, data_size_temp, data_disp_temp, MPI.DOUBLE])
                    x_bin = x_bin_temp.reshape(N_designs, -1)
                else:
                    data_size_temp = data_size*(Nx + 2*min_feature_size)*(Ny + 2*min_feature_size)*upsample_ratio**2
                    data_disp_temp = np.array([sum(data_size_temp[:p]) for p in range(comm.size)])
                    
                    x_bin_temp = np.zeros(N_designs*(Nx + 2*min_feature_size)*(Ny + 2*min_feature_size)*upsample_ratio**2)
                    comm.Allgatherv(x_bin_proc.reshape(-1), [x_bin_temp, data_size_temp, data_disp_temp, MPI.DOUBLE])
                    x_bin = x_bin_temp.reshape(N_designs, -1)
        
        t2 = time.time()
        #np.savez('debug_binarize', x_reward=x_reward, x_bin=x_bin, padding=padding)
        ...
        if padding is not None:
            x_bin_crop = x_bin.reshape(N_designs, (Nx + 2*min_feature_size)*upsample_ratio, (Ny + 2*min_feature_size)*upsample_ratio)
            x_bin_crop = x_bin_crop[:,min_feature_size:-min_feature_size,min_feature_size:-min_feature_size]
            x_bin = x_bin_crop.reshape(N_designs, -1)
