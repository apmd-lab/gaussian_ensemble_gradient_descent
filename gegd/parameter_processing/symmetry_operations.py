...
def symmetrize(x, symmetry, Nx, Ny):
    if symmetry == 0:
        ...
        assert Nx == Ny, "The pixel numbers in the X and Y directions must be equal under diagonal symmetry"
        
        x = x.reshape(-1)
        x_sym = np.zeros((Nx, Ny))
        x_sym[np.triu_indices(Nx)] = x
...
def symmetrize_jacobian(jac, symmetry, Nx, Ny):
    if symmetry == 0:
        ...
        scaling_coeff = (1/2)*np.ones((Nx, Ny))
        scaling_coeff += (1/2)*np.identity(Nx)
        scaling_coeff = scaling_coeff[np.triu_indices(Nx)]
...
def desymmetrize_jacobian(jac_sym, symmetry, Nx, Ny):
    if symmetry == 0:
        ...
        jac_sym += jac_sym.T - np.diag(np.diag(jac_sym))

        jac = jac_sym[np.triu_indices(Nx)]
